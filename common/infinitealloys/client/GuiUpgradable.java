...
public abstract class GuiUpgradable extends GuiContainer {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTick) {
		...
		if(mouseInZone(mouseX, mouseY, slot.xDisplayPosition + topLeft.x, slot.yDisplayPosition + topLeft.y, 16, 16)) {
			...
			for(int i = 0; i < Consts.UPGRADE_COUNT; i++) {
				...
				if(TEHelper.isPrereqUpgrade(upg) && teu.hasUpgrade(upg << 1) || !teu.hasUpgrade(upg))
					continue;
	...
	protected void drawGuiContainerBackgroundLayer(float partialTick, int mouseX, int mouseY) {
		...
		if(helpEnabled) {
			...
			for(MachineHelp help : MachineHelp.getBoxesForTEM(teu)) {
