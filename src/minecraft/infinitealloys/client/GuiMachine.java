...
public abstract class GuiMachine extends GuiContainer {
	...
	public void initGui() {
		super.initGui();
		controlList.clear();
		controlList.add(idButton = new GuiButton(0, width / 2 + 61, height / 2 - 102, 20, 20, new Byte(tem.networkID).toString()));
	}
	...
	protected void drawTextBox(List<String> text, int mouseX, int mouseY) {
		GL11.glPushMatrix();
		...
		if(!text.isEmpty()) {
			...
			for(int var13 = 0; var13 < text.size(); ++var13) {
				fontRenderer.drawStringWithShadow("\u00a7f" + text.get(var13), mouseX, mouseY, -1);
				if(var13 == 0)
					mouseY += 2;
				mouseY += 10;
			}
