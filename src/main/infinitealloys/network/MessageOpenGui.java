...
public class MessageOpenGui implements IMessage, IMessageHandler<MessageOpenGui, IMessage> {
	...
	public IMessage onMessage(MessageOpenGui message, MessageContext context) {
		...
		((BlockMachine)player.worldObj.getBlock(machine.x, machine.y, machine.z)).openGui(player.worldObj, player, (TileEntityMachine)player.worldObj.getTileEntity(machine.x, machine.y, machine.z));
