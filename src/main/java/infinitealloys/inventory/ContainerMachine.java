...
public abstract class ContainerMachine extends Container {
  ...
  /**
   * Used for machines that have their own Container class. {@link #initSlots} needs to be run
   * later
   */
  ...
  /**
   * Used for machines without their own Container class. {@link #initSlots} doesn't need to be run
   * later
   */
  public ContainerMachine(InventoryPlayer inventoryPlayer, TileEntityMachine tileEntity, int invX,
                          int invY, int upgX, int upgY) {
    inventory = tileEntity;
    initSlots(inventoryPlayer, invX, invY, upgX, upgY);
  }

  /**
   * Given the coordinates for the player's inventory and the upgrade slot, add these slots to the
   * container
   */
  protected void initSlots(InventoryPlayer inventoryPlayer, int invX, int invY, int upgX,
                           int upgY) {
    addSlotToContainer(
        new SlotUpgrade(inventory, inventory.upgradeSlotIndex, upgX, upgY)); // Add the upgrade slot
      ...
      // If an item was clicked in the main inventory or the hotbar
        ...
        for (int i = 0; i < inventoryStart; i++) {
          if (inventory.isItemValidForSlot(i, stackInSlotCopy)) {
            moved = true;
            if (!mergeItemStack(stackInSlotCopy, i, i + 1, false) && !inventory
                .isItemValidForSlot(i + 1, stackInSlotCopy)) {
              break;
            }
          }
        }
  ...
  @Override
  protected boolean mergeItemStack(ItemStack itemstack, int slotStart, int slotEnd,
                                   boolean backwards) {
    boolean var5 = false;
    ...
    if (itemstack.isStackable()) {
      while (itemstack.stackSize > 0 && (!backwards && currentSlot < slotEnd
                                         || backwards && currentSlot >= slotStart)) {
