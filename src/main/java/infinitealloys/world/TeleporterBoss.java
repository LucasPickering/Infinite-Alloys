...
public class TeleporterBoss extends Teleporter {
  ...
  public void placeInPortal(Entity par1Entity, double par2, double par4, double par6, float par8) {
    if (worldServerInstance.provider.dimensionId != 1) {
      ...
      par1Entity
          .setLocationAndAngles((double) i, (double) j, (double) k, par1Entity.rotationYaw, 0.0F);
  ...
  public boolean placeInExistingPortal(Entity entity, double par2, double par4, double par6,
      ...
      for (k1 = l - short1; k1 <= l + short1; ++k1) {
        ...
        for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
          ...
          for (int i2 = worldServerInstance.getActualHeight() - 1; i2 >= 0; --i2) {
            if (worldServerInstance.getBlock(k1, i2, l1) == IABlocks.portal) {
              while (worldServerInstance.getBlock(k1, i2 - 1, l1) == IABlocks.portal) {
                --i2;
              }
    ...
    if (d3 >= 0.0D) {
      ...
      if (worldServerInstance.getBlock(i - 1, j, k) == IABlocks.portal) {
        j2 = 2;
      }
      ...
      if (worldServerInstance.getBlock(i, j, k - 1) == IABlocks.portal) {
        j2 = 3;
      }
      ...
      if (j2 > -1) {
        ...
        boolean
            flag1 =
            !worldServerInstance.isAirBlock(i + i3 + k3, j, k + j3 + l3)
            || !worldServerInstance.isAirBlock(i + i3 + k3, j + 1, k + j3 + l3);
        boolean
            flag2 =
            !worldServerInstance.isAirBlock(i + i3, j, k + j3) || !worldServerInstance
        ...
        if (flag1 && flag2) {
          ...
          flag2 = !worldServerInstance.isAirBlock(k1 + i3, j, i4 + j3) || !worldServerInstance
              .isAirBlock(k1 + i3, j + 1, i4 + j3);
        }
    ...
    for (k2 = 0; k2 < 4; ++k2) {
      for (i3 = 0; i3 < 4; ++i3) {
        for (l2 = -1; l2 < 4; ++l2) {
          ...
          worldServerInstance.setBlock(k3, j3, i4, flag ? Blocks.sandstone : IABlocks.portal, 0, 2);
        }
      }
      ...
      for (i3 = 0; i3 < 4; ++i3) {
        for (l2 = -1; l2 < 4; ++l2) {
          ...
          j3 = j5 + l2;
          i4 = j2 + (i3 - 1) * l5;
          worldServerInstance.notifyBlocksOfNeighborChange(k3, j3, i4, worldServerInstance
              .getBlock(k3, j3, i4));
        }
      }
    }
